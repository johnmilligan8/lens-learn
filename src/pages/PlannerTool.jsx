...
function altAzToCanvas(alt, az, cx, cy, R) {
  ...
  return { x: cx + R * elev * Math.cos(rad), y: cy + R * elev * Math.sin(rad) };
}
...
function SkyCanvas({ gcData, lat, lon, dateStr }) {
  ...
  // Time scrubber: 0–23 in 0.25 increments (quarter hours), relative to midnight UTC
  const [scrubHour, setScrubHour] = useState(22); // default 10pm
  const [clickInfo, setClickInfo] = useState(null); // { name, alt, az, time }
  const [hovered, setHovered] = useState(null);

  // Build current-hour object positions
  const currentObjects = REFERENCE_OBJECTS.map(obj => {
    const { alt, az } = computeObjectPos(obj.ra, obj.dec, lat, lon, dateStr, scrubHour);
    const pos = altAzToCanvas(alt, az, cx, cy, R);
    return { ...obj, alt: Math.round(alt * 10) / 10, az: Math.round(az * 10) / 10, pos };
  });

  // GC arc path for the full day
  const gcArcPoints = gcData?.results ?? [];
  ...
  function draw() {
    ...
    // ── Altitude rings ──
    [[0.333, '30°'], [0.667, '60°'], [1, '90°']].forEach(([f, lbl]) => {
      ctx.beginPath();
      ctx.arc(cx, cy, R * f, 0, 2 * Math.PI);
      ctx.strokeStyle = 'rgba(100,80,160,0.18)';
      ctx.lineWidth = 1;
      ctx.setLineDash([4, 6]);
      ctx.stroke();
      ctx.setLineDash([]);
    ...
    [0, 45, 90, 135, 180, 225, 270, 315].forEach(az => {
      ...
      ctx.strokeStyle = 'rgba(100,80,160,0.07)';
      ctx.lineWidth = 1;
      ctx.stroke();
    });
    ...
    [['N', 0], ['NE', 45], ['E', 90], ['SE', 135], ['S', 180], ['SW', 225], ['W', 270], ['NW', 315]].forEach(([lbl, az]) => {
      ...
      const rad = toRad(az - 90);
      const dist = R + (isCard ? 11 : 9);
      const x = cx + dist * Math.cos(rad);
      const y = cy + dist * Math.sin(rad);
      ctx.fillStyle = isCard ? '#94a3b8' : 'rgba(100,116,139,0.5)';
    ...
    // ── GC full-day arc ──
    const visible = gcArcPoints.filter(p => p.alt > 0);
    if (visible.length > 1) {
      // Glow
      ctx.beginPath();
      let started = false;
    ...
    const starSeeds = [[0.15,0.25],[0.8,0.1],[0.4,0.7],[0.6,0.3],[0.2,0.6],[0.75,0.65],[0.5,0.15],[0.3,0.45],[0.65,0.5],[0.1,0.8],[0.9,0.4]];
    starSeeds.forEach(([fx, fy]) => {
      const x = cx + (fx - 0.5) * 2 * R * 0.9;
      const y = cy + (fy - 0.5) * 2 * R * 0.9;
      const dist = Math.hypot(x - cx, y - cy);
      if (dist > R * 0.96) return;
      ctx.beginPath();
    ...
    currentObjects.forEach(obj => {
      ...
      const { x, y } = obj.pos;
      // glow
      const grd = ctx.createRadialGradient(x, y, 0, x, y, obj.size * 2.5);
      grd.addColorStop(0, obj.color + 'aa');
      grd.addColorStop(1, 'transparent');
      ctx.beginPath();
      ctx.arc(x, y, obj.size * 2.5, 0, 2 * Math.PI);
      ctx.fillStyle = grd;
      ctx.fill();
      // dot
      ctx.beginPath();
      ctx.arc(x, y, obj.size * 0.7, 0, 2 * Math.PI);
      ...
      if (obj.isPrimary) {
        ...
        ctx.textAlign = 'center';
        ctx.fillText('GC', x, y - 11);
      }
    });

    // ── Current time cursor on GC arc ──
    // Find the closest GC data point to scrubHour
    const baseDate = new Date(dateStr + 'T00:00:00Z');
    const curTime = new Date(baseDate.getTime() + scrubHour * 3600000);
    const { alt: curAlt, az: curAz } = computeObjectPos(GC_RA, GC_DEC, lat, lon, dateStr, scrubHour);
    if (curAlt > 0) {
      const pos = altAzToCanvas(curAlt, curAz, cx, cy, R);
      if (pos) {
        ctx.beginPath();
    ...
    if (clickInfo?.canvasPos) {
      ...
      ctx.strokeStyle = 'rgba(56,189,248,0.9)'; ctx.lineWidth = 1.5; ctx.stroke();
    }
  }

  useEffect(() => { draw(); }, [gcData, scrubHour, clickInfo, lat, lon, dateStr]);

  function handleCanvasClick(e) {
    const canvas = canvasRef.current;
    const rect = canvas.getBoundingClientRect();
    // Scale click to canvas coords
    ...
    if (dist > R) return; // outside sky circle

    // Check if clicked near a reference object
    let closest = null, closestD = Infinity;
    currentObjects.forEach(obj => {
      if (!obj.pos) return;
      const d = Math.hypot(mx - obj.pos.x, my - obj.pos.y);
      if (d < 14 && d < closestD) { closest = obj; closestD = d; }
    });

    if (closest) {
      setClickInfo({ name: closest.name, alt: closest.alt, az: closest.az, canvasPos: closest.pos, isObject: true });
      return;
    }

    // Click on arbitrary sky point — reverse-project to alt/az
    const altFrac = dist / R; // 0 = zenith, 1 = horizon
    const alt = Math.round((1 - altFrac) * 90 * 10) / 10;
    let az = Math.round((toDeg(Math.atan2(dy, dx)) + 90 + 360) % 360 * 10) / 10;
  ...
  const gcNow = (() => {
    const { alt, az } = computeObjectPos(GC_RA, GC_DEC, lat, lon, dateStr, scrubHour);
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="relative">
        <canvas
          ...
          width={SIZE}
          height={SIZE}
          className="w-full max-w-[320px] mx-auto rounded-full block"
        ...
        />
        {hovered && (
          <div className="absolute top-2 left-2 bg-slate-900/90 border border-slate-700 rounded-lg px-2 py-1 text-xs text-cyan-300 pointer-events-none">
            {hovered}
          </div>
        )}
      </div>
      ...
      <div className={`rounded-lg px-3 py-2 text-xs border ${gcNow.visible ? 'bg-purple-900/20 border-purple-500/30' : 'bg-slate-800/40 border-slate-700/40'}`}>
        <span className="text-slate-400">Galactic Core at {timeLabel}: </span>
        {gcNow.visible
          ...
          : <span className="text-slate-500 font-semibold">Below horizon</span>
        }
      </div>
      ...
      {clickInfo && (
        <div className="rounded-lg px-3 py-2 text-xs border bg-cyan-900/20 border-cyan-500/30">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setClickInfo(null)} className="text-slate-500 hover:text-slate-300 text-xs">✕</button>
          </div>
          <div className="flex gap-4 mt-1 text-slate-300">
            ...
            {clickInfo.isObject && <span className="text-emerald-400">{clickInfo.alt > 0 ? '● Visible' : '○ Below horizon'}</span>}
          </div>
        </div>
      )}

      <p className="text-xs text-slate-600 text-center">Click any object or sky point for coordinates</p>
    </div>
  );
}
