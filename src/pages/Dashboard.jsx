...
export default function Dashboard() {
  ...
  return (
    <PullToRefresh onRefresh={loadData}>
      ...
      {!isSubscribed && (
        <div className="mb-8 p-5 rounded-2xl border border-red-700/40 bg-[#1a1a1a] flex flex-col md:flex-row items-start md:items-center gap-4">
          <div className="flex-1">
            <p className="text-white font-black text-lg leading-snug">🚀 Ready to master the stars?</p>
            <p className="text-slate-400 text-sm mt-1">Unlock full planning tools, guided shoot plans, gear checklists & aurora alerts — from <strong className="text-white">$7.99/month</strong>. Or go Pro at $14.99/mo for instructor access & journal insights.</p>
          </div>
      ...
      {!isSubscribed && (
        <div className="mb-8">
          ...
          <Link to={createPageUrl('FreeCourse')}>
            <Card className="bg-[#1a1a1a] border border-white/8 hover:border-red-600/40 p-6 transition-all group">
              <div className="flex items-start gap-5">
                <div className="bg-red-600/10 border border-red-700/30 p-4 rounded-xl flex-shrink-0">
            ...
            {modules.map((mod) => {
              ...
              return (
                <Link key={mod.id} to={isSubscribed ? createPageUrl('ModuleView') + `?id=${mod.id}` : createPageUrl('PaymentGate')}>
                  <Card className={`bg-[#1a1a1a] border border-white/8 hover:border-red-600/40 p-6 hover:scale-[1.02] transition-all duration-200 relative overflow-hidden h-full ${!isSubscribed ? 'opacity-60' : ''}`}>
                    {mod.is_free_preview && (
                      <Badge className="absolute top-3 right-3 bg-red-600 text-white text-xs">FREE</Badge>
                    )}
                    <div className="flex items-start justify-between mb-4">
                      <IconComp className="w-10 h-10 text-red-400/70" />
